defmodule CredoAsh.QueryAndActorTest do
  ...
  describe "UnjustifiedAuthorizeFalse" do
    ...
    test "does not treat an empty trailing comment as a justification" do
      """
        def all do
          Ash.read!(MyApp.Blog.Post, authorize?: false) #
        end
    ...
    test "does not treat an empty comment line as a justification" do
      """
        def all do
          #
          Ash.read!(MyApp.Blog.Post, authorize?: false)
    ...
    test "accepts an explanation above the function head that contains an empty comment line" do
      """
        # Public listing: the posts are visible to everyone by design.
        #
        # Policies are skipped so the page renders without a session.
        def all do
          Ash.read!(MyApp.Blog.Post, authorize?: false)
        end
      """
