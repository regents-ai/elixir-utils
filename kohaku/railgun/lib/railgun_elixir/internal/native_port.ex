defmodule RailgunElixir.Internal.NativePort do
  @moduledoc false
  ...
  @native_manifest Path.expand("../../../native/railgun_native/Cargo.toml", __DIR__)
  @debug_executable Path.expand(
                      "../../../native/railgun_native/target/debug/railgun_native",
                      __DIR__
                    )
  @release_executable Path.expand(
                        "../../../native/railgun_native/target/release/railgun_native",
                        __DIR__
                      )
  ...
  def init(opts) do
    ...
    case File.exists?(executable) do
      ...
      false ->
        {:stop,
         {:railgun_native_missing,
          "build native bridge with `mix native.build` or set RAILGUN_NATIVE_EXECUTABLE"}}
    end
  end
  ...
  def handle_call({:request, op, params, timeout}, from, state) do
    ...
    case Port.command(state.port, payload) do
      ...
      false ->
        {:reply, {:error, Error.native("native bridge is not available", %{})}, state}
    end
  end
  ...
  def handle_info({_port, {:exit_status, status}}, state) do
    Enum.each(state.requests, fn {_id, {from, timer}} ->
      ...
      GenServer.reply(from, {:error, Error.native("native bridge exited", %{status: status})})
    end)
  ...
  def handle_info({:request_timeout, id}, state) do
    case Map.pop(state.requests, id) do
      {{from, _timer}, requests} ->
        GenServer.reply(
          from,
          {:error, Error.native("native bridge request timed out", %{id: id})}
        )
    ...
    with {:ok, payload} <- Jason.decode(line),
      ...
      reply =
        if ok? do
          ...
          {:error,
           Error.native(Map.get(payload, "error", "native bridge request failed"), %{id: id})}
        end
  ...
  defp configured_executable do
    cond do
      value = System.get_env("RAILGUN_NATIVE_EXECUTABLE") ->
        value
