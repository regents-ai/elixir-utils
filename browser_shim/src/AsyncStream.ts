...
const usableProperties = ["end", "isDone", "next", "return", Symbol.asyncIterator] as const;

const isUsableProperty = <T>(
  prop: string | symbol,
): prop is keyof AsyncStreamProxy<T> => {
  return usableProperties.includes(prop as (typeof usableProperties)[number]);
};
...
export function createAsyncStreamProxy<T>(stream: AsyncStream<T>) {
  return new Proxy(stream, {
    ...
    ownKeys() {
      return usableProperties as unknown as ArrayLike<string | symbol>;
    },
